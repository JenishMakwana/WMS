...
export default function DeliveriesPage() {
  ...
  const { data: customers = [] } = useQuery({ queryKey: ["customers"], queryFn: () => customerApi.list(), enabled: showModal });

  const { register, control, handleSubmit, reset } = useForm({
    defaultValues: { customer_id: "", src_location_id: "", scheduled_date: "", moves: [{ product_id: "", demand_qty: "" }] },
  });
  const { fields, append, remove } = useFieldArray({ control, name: "moves" });

  const buildDeliveryPayload = (data: any) => {
    const selectedCustomer = customers.find((c) => c.id === data.customer_id);
    return {
      type: "DELIVERY" as const,
      customer: selectedCustomer?.name || undefined,
      customer_id: data.customer_id || undefined,
      src_location_id: data.src_location_id || undefined,
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-3">
            ...
            <Button className="w-auto px-4" onClick={() => setShowModal(true)}>+ New delivery</Button>
          </div>
        }
      />
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button onClick={() => setShowModal(true)} className="mt-3 text-sm text-brand-400 hover:text-brand-100">Create your first delivery</button>
          </div>
        ) : (
              ...
              {deliveries.map((d) => (
                <tr key={d.id} className="hover:bg-gray-800/40 transition-colors">
                  <td className="py-3 pr-4">
                    <Link to={`/operations/${d.id}`} className="font-mono text-brand-400 hover:text-brand-100 transition-colors">{d.reference}</Link>
                  </td>
                  <td className="py-3 pr-4 text-gray-300">{d.customer_name || d.customer || "—"}</td>
                  <td className="py-3 pr-4 text-gray-400">{d.src_location_name ?? "—"}</td>
                  <td className="py-3 pr-4 text-gray-400">{d.move_count}</td>
                  <td className="py-3 pr-4 text-gray-400">{formatDate(d.scheduled_date ?? d.created_at)}</td>
                  <td className="py-3"><StatusBadge status={d.status} /></td>
                  <td className="py-3 text-right pr-4">
                    <button
                      onClick={() => handlePreview(d.id, d.reference)}
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); reset(); }} title="New delivery" width="max-w-2xl">
        <form
          onSubmit={handleSubmit((d) => createMutation.mutate(buildDeliveryPayload(d)))}
          className="flex flex-col gap-4"
        >
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-gray-300">Customer</label>
              <select className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3.5 py-2.5 text-sm text-gray-100 outline-none focus:border-brand-400" {...register("customer_id")}>
                <option value="">— Select —</option>
                {customers.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-gray-300">Source location</label>
              <select className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3.5 py-2.5 text-sm text-gray-100 outline-none focus:border-brand-400" {...register("src_location_id")}>
                <option value="">— Select —</option>
                {locations.map((l) => <option key={l.id} value={l.id}>{l.warehouse_name} / {l.name}</option>)}
              </select>
            </div>
          </div>
          <Input label="Delivery date" type="date" {...register("scheduled_date")} />

          <div className="border-t border-gray-800 pt-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-medium text-gray-300">Products</p>
              <button type="button" onClick={() => append({ product_id: "", demand_qty: "" })} className="text-xs text-brand-400 hover:text-brand-100">+ Add line</button>
            </div>
            {fields.map((field, i) => (
              <div key={field.id} className="flex gap-3 mb-2 items-end">
                ...
                {fields.length > 1 && (
                  <button type="button" onClick={() => remove(i)} className="mb-0.5 text-red-400 hover:text-red-300">
          ...
          {createMutation.isError && (
            <p className="rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-400 border border-red-500/20">
              {((createMutation.error as any)?.response?.data?.detail) || "Failed to create delivery."}
            </p>
          )}
